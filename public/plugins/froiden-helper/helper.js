(function($) {
    ...
    $.easyAjax = function(options) {
        ...
        function handleFail(response) {
            $(opt.container).append('<div id="errorName" data-name="no-error"></div>');
            if (typeof response.message != "undefined") {
            ...
            if (typeof response.errors != "undefined") {
                ...
                if (opt.errorPosition == "field") {
                    ...
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        $(opt.container).find("[name='" + keyWithArray + "']").each(function(keyArrayItem,valueArrayItem){
                            if(keyArrayItem == keyIndex){
                                ele = $(this);
                                var grp = ele.closest(".form-group");
                                $(grp).find(".help-block").remove();
